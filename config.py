...
# Через сколько минут после НАЧАЛА визита он автоматически считается состоявшимся
# (active -> completed), если администратор не отметил иначе (неявка).
ATTENDANCE_GRACE_MINUTES = 120
# За сколько часов до визита клиент ещё может сам отменить/перенести.
# 0 = без ограничения (ближе к визиту просим звонить).
CLIENT_CHANGE_CUTOFF_HOURS = 0
